...
import (
	...
	"go.aoach.tech/bouncer/pkg/bouncer"
)
...
func NewRootCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			bouncer.Out = cmd.OutOrStdout()
			bouncer.Err = cmd.ErrOrStderr()

			return initializeConfig(cmd)
		},
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Fprint(bouncer.Out, cmd.UsageString())
		},
	}

	cmd.PersistentFlags().BoolVar(&bouncer.NoExitCode, "no-exit-on-fail", false, "Don't return a non-zero exit code on failure.")
